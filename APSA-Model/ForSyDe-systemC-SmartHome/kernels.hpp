...
void bi_kernel_func(
    std::tuple<std::vector<std::vector<double>>, std::vector<double>>& out,
    const simple_scenario_type& sc,
    const std::tuple<std::vector<double>>& inp)
{
    const auto& t_room_vec = std::get<0>(inp);   

    std::cout << ">>> [BI] fired\n";
    std::cout << "    t_room tokens: size=" << t_room_vec.size() << "  values=[";
    for (size_t i = 0; i < t_room_vec.size(); ++i)
        std::cout << (i? ", ":"") << t_room_vec[i];
    std::cout << "]\n";

    auto& outTemps = std::get<0>(out);  // vector< vector<double> >
    auto& outAvg   = std::get<1>(out);  // vector<double>
    ...
    if (sc == SelfAware_OPERATE)
    {
        ...
        {
            std::cout << "    [BI] not enough tokens (need 5). Produced nothing.\n";
            return; 
        }
        ...
        std::cout << "    [BI] avg=" << avg << "  rate=" << rate << "\n";
        std::cout << "    [BI] out[0]=[avg,rate]=[" << outTemps[0][0] << ", " << outTemps[0][1] << "]\n";
        std::cout << "    [BI] out[1]=[avg]=[" << outAvg[0] << "]\n";
    }
    else
    {
        std::cout << "    [BI] normal_OPERATE -> no outputs (rates = 0)\n";
    }
...
void bo_kernel_func(
    std::tuple<std::vector<std::vector<double>>>& out,
    const simple_scenario_type& sc,
    const std::tuple<std::vector<std::vector<double>>,
                     std::vector<std::vector<double>>>& inp)
{
    auto& out_sp = std::get<0>(out);
    out_sp.clear();

    std::cout << ">>> [BO] fired. scenario="
              << (sc == SelfAware_OPERATE ? "SelfAware" : "normal") << "\n";

    if (sc != SelfAware_OPERATE) {
        std::cout << "    [BO] normal -> no outputs (rates=0)\n";
        return;
    }

    const auto& al_sp = std::get<0>(inp); // forward this unchanged

    if (al_sp.empty() || al_sp[0].size() < 2) {
        std::cout << "    [BO][WARN] AL setpoint malformed; no outputs.\n";
        return;
    }
    ...
    std::cout << "    [BO] pass-through: sp=[clg=" << out_sp[0][0]
              << ", htg=" << out_sp[0][1] << "]\n"
              << ">>> [BO] end\n";
}
 
void d1_kernel_func(
    std::tuple<std::vector<double>, std::vector<double>>& out,
    const simple_scenario_type& sc,
    const std::tuple<std::vector<std::vector<double>>, std::vector<double>>& inp)
{
    auto& out_to_E_AL_vec    = std::get<0>(out);
    auto& out_to_J_AL_AL_vec = std::get<1>(out);

    // reset outputs; will resize according to production rates
    out_to_E_AL_vec.clear();
    out_to_J_AL_AL_vec.clear();

    std::cout << ">>> [D1] fired. scenario="
              << (sc == SelfAware_OPERATE ? "SelfAware" : "normal") << "\n";

    if (sc != SelfAware_OPERATE)
    {
        std::cout << "    [D1] normal_OPERATE -> no outputs (rates=0)\n";
        return; // if your scenario table requires outputs here, adjust accordingly
    }

    // ---- inputs ----
    const auto& bi_pack = std::get<0>(inp);  // [[mean, speed]]
    const auto& eal_vec = std::get<1>(inp);  // [state]

    if (bi_pack.empty() || bi_pack[0].size() < 2)
    {
        std::cout << "    [D1][WARN] bi_pack malformed (need [[mean, speed]]) "
                  << "bi_pack.size()=" << bi_pack.size();
        if (!bi_pack.empty())
            std::cout << " bi_pack[0].size()=" << bi_pack[0].size();
        std::cout << " -> no outputs.\n";
        return;
    }
    if (eal_vec.empty())
    {
        std::cout << "    [D1][WARN] eal_vec empty (need [state]) -> no outputs.\n";
        return;
    }
    ...
    std::cout << "    [D1] inputs: BI_mean=" << bi_mean
              << "  BI_speed=" << bi_speed
    ...
    if (bi_mean <= winter_max) {
        ...
        std::cout << "    [D1] rule: mean <= winter_max (" << bi_mean << " <= " << winter_max << ") -> WINTER\n";
    } else if (bi_mean >= summer_min) {
        inferred_season = SEASON_SUMMER;
        std::cout << "    [D1] rule: mean >= summer_min (" << bi_mean << " >= " << summer_min << ") -> SUMMER\n";
    } else {
        // tie-band: (winter_max, summer_min)
        if (bi_speed >  speed_thresh) {
            inferred_season = SEASON_WINTER;
            std::cout << "    [D1] tie-band: speed > +" << speed_thresh << " -> WINTER\n";
        } else if (bi_speed < -speed_thresh) {
            inferred_season = SEASON_SUMMER;
            std::cout << "    [D1] tie-band: speed < -" << speed_thresh << " -> SUMMER\n";
        } else {
            inferred_season = model_season;
            std::cout << "    [D1] tie-band: |speed| <= " << speed_thresh << " -> keep MODEL\n";
        }
    }
    ...
    if (inferred_season != SEASON_UNKNOWN && inferred_season != model_season) {
        ...
        mismatch_flag = 1.0;
        std::cout << "    [D1] mismatch: model=" << model_season
                  << " inferred=" << inferred_season
                  << " -> send inferred, flag=1\n";
    } else {
        out_to_E_AL   = static_cast<double>(model_season);
        mismatch_flag = 0.0;
        std::cout << "    [D1] agree/unknown -> keep model=" << model_season
                  << " flag=0\n";
    }
    ...
    std::cout << "    [D1] out_to_E_AL=" << out_to_E_AL
...
void d2_kernel_func(
    ...
    const std::tuple<std::vector<std::vector<double>>,
                     std::vector<std::vector<double>>>& inp)
{
    auto& out_sp     = std::get<0>(out); // [[clg, htg]]
    auto& out_ok_aid = std::get<1>(out); // [ok_flag] (aux / optional)
    auto& out_flag   = std::get<2>(out); // [ok_flag] -> J_AL_AL
    ...
    std::cout << ">>> [D2] fired. scenario="
              << (sc == SelfAware_OPERATE ? "SelfAware" : "normal") << "\n";

    if (sc != SelfAware_OPERATE) {
        std::cout << "    [D2] normal -> no outputs (rates=0)\n";
        return;
    }

    const auto& sp_pack   = std::get<0>(inp); // expect [[clg, htg]]
    const auto& abs_pack  = std::get<1>(inp); // expect [[abs_clg, abs_htg]]

    if (sp_pack.empty() || sp_pack[0].size() < 2) {
        std::cout << "    [D2][WARN] setpoint input malformed; no outputs.\n";
        return;
    }
    if (abs_pack.empty() || abs_pack[0].size() < 2) {
        std::cout << "    [D2][WARN] absolute bounds input malformed; no outputs.\n";
        return;
    }
    ...
    std::cout << "    [D2] inp: sp=[clg=" << clg << ", htg=" << htg
    ...
    if (!(abs_clg < abs_htg)) {
        std::cout << "    [D2] invalid abs range (abs_clg >= abs_htg)\n";
        logical = false;
    }

    // current setpoints must be ordered clg < htg
    if (!(clg < htg)) {
        std::cout << "    [D2] invalid sp ordering (clg >= htg)\n";
        logical = false;
    }

    // current setpoints must lie inside absolute window
    if (clg < abs_clg) {
        std::cout << "    [D2] clg < abs_clg -> out of bounds\n";
        logical = false;
    }
    if (htg > abs_htg) {
        std::cout << "    [D2] htg > abs_htg -> out of bounds\n";
        logical = false;
    }
    ...
    std::cout << "    [D2] outputs: pass sp=[clg=" << out_sp[0][0]
              << ", htg=" << out_sp[0][1] << "], ok=" << ok_flag << "\n"
              << ">>> [D2] end\n";
}

void J_AL_kernel_func(
    std::tuple<std::vector<double>, std::vector<double>, std::vector<double>>& out,  // out[0]=activate?, out[1]=distance, out[2]=stagnation
    const simple_scenario_type& sc,
    const std::tuple<std::vector<double>, std::vector<std::vector<double>>, std::vector<double>>& inp) // inp[0]=[T_room], inp[1]=[[sp_low, sp_high]], inp[2]=[prev_stag]
{
    auto& out_flag  = std::get<0>(out);
    auto& out_dist  = std::get<1>(out);
    auto& out_stag  = std::get<2>(out);
    ...
    std::cout << ">>> [J_AL] fired. scenario="
              << (sc == SelfAware_OPERATE ? "SelfAware" : "normal") << "\n";

    // Soft-input checks (avoid OOB)
    const auto& vT = std::get<0>(inp);
    const auto& vSP = std::get<1>(inp);
    const auto& vPrev = std::get<2>(inp);

    if (vT.empty() || vSP.empty() || vSP[0].size() < 2 || vPrev.empty()) {
        std::cout << "    [J_AL][WARN] malformed inputs. "
                  << "sizes: T=" << vT.size()
    ...
    std::cout << "    [J_AL] inputs: Tin=" << T_room
              << "  sp_low=" << sp_low
              << "  sp_high=" << sp_high
              << "  prev_stag=" << prev_in << "\n";
    ...
    if (!initialized) {
        ...
        stagnation_count = std::max(0, prev_in);  // initialize from prev if you like; else 0
        initialized      = true;
        std::cout << "    [J_AL] init: last_distance=" << last_distance
        ...
        std::cout << "    [J_AL] distance=" << distance
                  << "  stagnation_count=" << stagnation_count << "\n";
    }
    ...
    std::cout << "    [J_AL] outputs: flag=" << out_flag[0]
              << "  distance=" << out_dist[0]
...
void AL_kernel_func(
    std::tuple<std::vector<setpoint>, std::vector<setpoint>>& out,        // out[0] -> E, out[1] -> BO
    const simple_scenario_type& sc,
    const std::tuple<std::vector<double>, std::vector<double>>& inp)       // inp[0]=[Tin], inp[1]=[distance]
{
    auto& out_to_E  = std::get<0>(out);
    auto& out_to_BO = std::get<1>(out);

    // clear; will resize as we produce
    out_to_E.clear();
    out_to_BO.clear();

    std::cout << ">>> [AL] fired. scenario="
              << (sc == SelfAware_OPERATE ? "SelfAware" : "normal") << "\n";

    const auto& troom_vec    = std::get<0>(inp);
    const auto& distance_vec = std::get<1>(inp);

    if (troom_vec.empty() || distance_vec.empty()) {
        std::cout << "    [AL][WARN] missing inputs -> no outputs.\n";
        return;
    }
    ...
    clg = std::clamp(clg, 5.0, 35.0);
    htg = std::clamp(htg, 5.0, 40.0);
...
void A_ALL_kernel_func(
    std::tuple<std::vector<double>, std::vector<std::vector<double>>>& out,
    const simple_scenario_type& sc,
    const std::tuple<std::vector<double>, std::vector<double>, std::vector<double>>& inp)
{
    auto& out_season  = std::get<0>(out); // vector<double>
    auto& out_sp_list = std::get<1>(out); // vector< setpoint=vector<double>{clg,htg} >

    // default: clear outputs; we will resize according to production rates
    out_season.clear();
    out_sp_list.clear();

    std::cout << ">>> [A_ALL] fired. scenario="
              << (sc == SelfAware_OPERATE ? "SelfAware" : "normal") << "\n";

    if (sc != SelfAware_OPERATE) {
        std::cout << "    [A_ALL] normal -> no outputs (rates=0)\n";
        return;
    }

    // inputs
    const auto& bi_mean_vec   = std::get<1>(inp);
    const auto& season_vec    = std::get<0>(inp);
    const auto& feedback_vec  = std::get<2>(inp); // unused here; just log size

    if (bi_mean_vec.empty() || season_vec.empty()) {
        std::cout << "    [A_ALL][WARN] missing inputs: "
    ...
    std::cout << "    [A_ALL] inputs: BI_mean=" << bi_mean
    ...
    default:
        ...
        std::cout << "    [A_ALL] UNKNOWN: keep previous clg=" << cooler_setpoint
                  << " htg=" << heater_setpoint << "\n";
        break;
    }
    ...
    std::cout << "    [A_ALL] outputs: season=" << out_season[0]
...
void JAL_AL_kernel_func(
    std::tuple<std::vector<double>, std::vector<double>, std::vector<double>>& out,        // out[0]=deactivate, out[1]=report, out[2]=status
    const simple_scenario_type& sc,
    const std::tuple<std::vector<double>, std::vector<double>, std::vector<double>>& inp)  // inp[0]=det1, inp[1]=det2, inp[2]=prev
{
    auto& out_deactivate = std::get<0>(out);
    auto& out_report     = std::get<1>(out);
    auto& out_status     = std::get<2>(out);

    out_deactivate.clear();
    out_report.clear();
    out_status.clear();

    std::cout << ">>> [J_AL_AL] fired. scenario =" << (sc == SelfAware_OPERATE ? "SelfAware" : "normal") << "\n";

    if (sc != SelfAware_OPERATE) {
        std::cout << "    [J_AL_AL] normal -> no outputs (rates=0)\n";
        return;
    }
    ...
    const auto& v0 = std::get<0>(inp);
    const auto& v1 = std::get<1>(inp);
    const auto& v2 = std::get<2>(inp);

    bool d1_ok   = (v0[0] == 0.0);  // D1 mismatch flag: 0 = OK
    bool d2_ok   = (v1[0] == 0.0);  // D2 OK flag:       0 = OK
    bool prev_ok = (v2[0] == 0.0);  // previous OK:      0 = OK

    std::cout << "    [J_AL_AL] interpreted: d1_ok=" << d1_ok
              << " d2_ok=" << d2_ok
              << " prev_ok=" << prev_ok << "\n";
    ...
    std::cout << "    [J_AL_AL] outputs: deactivate=" << out_deactivate[0]
              << " report=" << out_report[0]
              << " status=" << out_status[0] << "\n"
              << ">>> [J_AL_AL] end\n";
}
...
void E_AL_kernel_func(
    std::tuple<std::vector<double>, std::vector<double>>& out,
    const simple_scenario_type& sc,
    const std::tuple<std::vector<double>, std::vector<double>>& inp)
{
    auto& out_to_D1  = std::get<0>(out); // season mode to D1 (vector<double>)
    auto& out_to_AALL= std::get<1>(out); // season mode to A_ALL (vector<double>)

    // Clear outputs by default; we’ll resize according to production rates
    out_to_D1.clear();
    out_to_AALL.clear();

    std::cout << ">>> [E_AL] fired. scenario="
              << (sc == SelfAware_OPERATE ? "SelfAware" : "normal") << "\n";

    if (sc != SelfAware_OPERATE)
    {
        std::cout << "    [E_AL] normal -> no outputs (rates=0)\n";
        return;
    }

    // Inputs
    const auto& d1_suggestion_vec = std::get<0>(inp); // expected size >= 1 (suggested season code)
    const auto& aall_update_vec   = std::get<1>(inp); // optional (not used here, but logged)

    double suggested = 0.0; // 0=UNKNOWN, 1=WINTER, 2=SUMMER
    if (!d1_suggestion_vec.empty())
        suggested = d1_suggestion_vec[0];

    std::cout << "    [E_AL] inp: suggested=" << suggested << "\n";
    ...
    if (!initialized)
    {
        ...
        if (suggested == 1.0 || suggested == 2.0)
        {
            ...
            initialized = true;
            std::cout << "    [E_AL] init: adopt suggestion -> season_mode = " << season_mode << "\n";
        }
        else
        {
            // remain UNKNOWN until a valid suggestion arrives
            std::cout << "    [E_AL] init: no valid suggestion, keep UNKNOWN\n";
        }
    }
    else
    {
        ...
        if (suggested == 1.0 || suggested == 2.0)
        {
            ...
            {
                std::cout << "    [E_AL] update: change season_mode "
                          << season_mode << " -> " << suggested << "\n";
                season_mode = suggested;
            }
            else
            {
                std::cout << "    [E_AL] update: suggestion equals current mode (" 
                          << season_mode << "), keep\n";
            }
        }
        else
        {
            std::cout << "    [E_AL] update: invalid/unknown suggestion (" 
    ...
    std::cout << "    [E_AL] outputs: to_D1=" << out_to_D1[0]

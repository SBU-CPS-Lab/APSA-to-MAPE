...
SC_MODULE(top)
{
    ...
    SADF::signal<std::vector<double>> s_BI_2D1 ;
    ...
    SC_CTOR(top)
    {
        ...
        // dalay s_from_AL
        SDF::make_delay("delay:setpoint",std::vector({18.0,22.0}) , Ds_from_AL, s_from_AL);

        // SADF::make_socketwrap("E", -1, std::string("0.0.0.0"), 9000, s_T_room, s_AL_to_E);
        ...
        []( double& t ,  const double& k) {
            ...
            t = Tmean + Amp * std::sin(2.0 * M_PI * static_cast<double>(k) / period);
            std::cout<<"in E :: tempreture -> "<<t<<std::endl;
        },
        22.0,
        6,        // number of samples
        ...
        "E_act_sink",
        [] (const setpoint& sp) {
            double htg = (sp.size() > 0 ? sp[0] : std::numeric_limits<double>::quiet_NaN());
            double clg = (sp.size() > 1 ? sp[1] : std::numeric_limits<double>::quiet_NaN());
            std::cout << "[E_STUB] setpoint -> htg=" << htg << "  clg=" << clg << std::endl;
        },
        ...
        //dalay s_D2_to_AALL
        SDF::make_delay("delay:s_D2_to_BO", std::vector({18.0,22.0}), Ds_BO_to_D2, s_BO_to_D2); 
        ...
        SDF::make_sink("F_report",[] (const double& v)
            {
                std::cout << "[REPORT] mismatch_summary = " << v << std::endl;
            },
            s_JALAL_report
        );
    }
